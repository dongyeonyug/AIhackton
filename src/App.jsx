...
function App() {
  ...
  function generateExam() {
    if (!activeProfessor) return;
    const style = personalityMap[activeProfessor.personality];
    const keywords = extractKeywords([examHints, materialNotes, ...state.materials.map((item) => item.text), activeProfessor.course].join("\n"));
    setExamOutput([
      ...
      <aside className={styles.sidebar}>
        ...
        <nav className={styles["nav-tabs"]}>
          {[
            ...
            <button key={key} className={view === key ? styles.active : ""} onClick={() => {
...
function ExamView({ materialNotes, setMaterialNotes, examHints, setExamHints, handleMaterialFiles, generateExam, examOutput }) {
  return (
    <>
